...
WebIM.conn.listen({
  ...
  onPresence: (msg) => {
    ...
    switch (msg.type) {
      ...
      case "leaveGroup": // 某人离开群
        message.error(`${msg.from}${I18n.t("LeaveGroup")}${msg.gid}.`);
        store.dispatch(
          GroupActions.removeGroupMemberAttr({
            groupId: msg.gid,
            uid: msg.from
          })
        );
      ...
      case "removedFromGroup":
        message.error(
          `${msg.kicked || I18n.t("you")} ${I18n.t("dismissed")}${I18n.t(
            "by"
          )}${msg.owner || I18n.t("admin")} .`
        );
        store.dispatch(GroupActions.getGroups());
        store.dispatch(MessageActions.clearUnread("groupchat", msg.gid));
        break;
      ...
      case "direct_joined": //被拉进群
        message.success(
          `${msg.from}${I18n.t("invite")}${I18n.t("you")}${I18n.t("join")}${
            msg.gid
          }`
        );
        store.dispatch(GroupActions.getGroups());
      ...
      case "joinPublicGroupSuccess":
        message.success(
          `${I18n.t("joinGroup")} ${msg.from} ${I18n.t("successfully")}`
        );
        store.dispatch(GroupActions.getGroups());
        break;
      case "joinPublicGroupDeclined":
        message.error(
          `${I18n.t("join")}${I18n.t("group")}${msg.gid}${I18n.t(
            "refuse"
          )}${I18n.t("by")}${msg.owner}`
        );
        break;
      ...
      case "memberJoinPublicGroupSuccess":
        message.success(
          `${msg.from}${I18n.t("join")}${I18n.t("group")}${msg.gid}${I18n.t(
            "successfully"
          )}`
        );
        store.dispatch(
          GroupMemberActions.listGroupMemberAsync({ groupId: msg.gid })
        );
        break;
      case "memberJoinChatRoomSuccess":
        message.success(
          `${msg.from}${I18n.t("join")}${I18n.t("chatroom")}${msg.gid}${I18n.t(
            "successfully"
          )}`
        );
        break;
  ...
  onError: (error) => {
    ...
    if (
      error.type == WebIM.statusCode.WEBIM_CONNCTION_USER_KICKED_BY_OTHER_DEVICE
    ) {
  ...
  onContactDeleted: (msg) => {
    console.log("onContactDeleted", msg);
  },
  onContactAdded: (msg) => {
    console.log("onContactAdded", msg);
  },
  onContactRefuse: (msg) => {
    console.log("onContactRefuse", msg);
  },
  onContactAgreed: (msg) => {
  ...
  onTextMessage: (message) => {
    ...
    if (
      type === "chat" &&
      message.from != "huanhuan" &&
      (_.get(
        rootState,
        "entities.roster.byName[" + chatId + "].subscription"
      ) === "none" ||
        !_.get(
          rootState,
          "entities.roster.byName[" + chatId + "].subscription"
      ...
      store.dispatch(
        StrangerActions.updateStrangerMessage(from, message, "txt")
      );
    }
    if (type === "groupchat") {
      ...
      if (mentionList && message.from !== WebIM.conn.user) {
        if (
          mentionList === MENTION_ALL ||
          mentionList.includes(WebIM.conn.user)
        ) {
    ...
    switch (type) {
      case "chat":
        ...
        if (
          ...
          message &&
          message.ext &&
          message.ext.msg_extension
        ) {
          var msgExtension =
            typeof message.ext.msg_extension == "string"
              ? JSON.parse(message.ext.msg_extension)
              : message.ext.msg_extension;
          var options = {
        ...
        if (message.ext && message.ext.action === "invite") {
          ...
          if (callVideo.callStatus > CALLSTATUS.idle) {
            // 正忙
            if (message.ext.callId == callVideo.callId) {
              // 多人会议中邀请别人
              store.dispatch(
                VideoCallAcctions.sendAlerting(
                  from,
                  message.ext.callerDevId,
                  message.ext.callId
                )
              ); // 回复alerting消息
              store.dispatch(
                VideoCallAcctions.setCallStatus(CALLSTATUS.alerting)
              ); // 更改为alerting状态
            } else {
              return store.dispatch(
                VideoCallAcctions.answerCall("busy", {
    ...
    if (msg.action === "rtcCall") {
      ...
      switch (msgInfo.action) {
        ...
        case "confirmRing":
          ...
          if (
            !msgInfo.status &&
            callVideo.callStatus < CALLSTATUS.receivedConfirmRing
          ) {
            console.warn("邀请已失效");
            store.dispatch(VideoCallAcctions.setCallStatus(CALLSTATUS.idle));
          ...
          deviceId = msgInfo.calleeDevId;
          store.dispatch(
            VideoCallAcctions.setCallStatus(CALLSTATUS.receivedConfirmRing)
          );
        ...
        case "answerCall":
          ...
          if (msgInfo.result !== "accept") {
            ...
            if (callVideo.confr.type !== 2) {
              // 单人情况挂断，多人不挂断
              store.dispatch(
                VideoCallAcctions.confirmCallee(
                  msg.from,
                  deviceId,
                  msgInfo.result
                )
        ...
        case "confirmCallee":
          ...
          store.dispatch(
            VideoCallAcctions.setCallStatus(CALLSTATUS.confirmCallee)
          );
          break;
        case "cancelCall":
...
WebIM.conn.addEventHandler("event", {
  onGroupEvent: ({ id, userId, attributes, operation }) => {
    if (operation === "memberAttributesUpdate") {
      store.dispatch(
        GroupActions.setGroupMemberAttr({
          groupId: id,
          attributes: {
            [userId]: attributes
          }
...
const { Types, Creators } = createActions({
  ...
  logout: () => {
    return (dispatch, state) => {
      let I18N =
        store.getState().i18n.translations[store.getState().i18n.locale];
