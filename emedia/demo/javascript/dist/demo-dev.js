...
/***/ function(module, exports, __webpack_require__) {
	...
	if (__super_tkt__) {
	    ...
	    var tktUrl = "/confr/superticket/?forward=" + __forward__ + "&confrId=" + confrId + "&invisible=" + __invisible__;
	...
	function genOrUpdateHtml(stream, update) {
	    ...
	    if (stream.type == 1) {
	        ...
	        btns += "<div id='desc'></div>";
	        btns += "<div><input type='checkbox' id='audio' disabled/><label for='audio'>Audio</label>" + "<input type='checkbox' id='video' /><label for='video'>Video</label></div>";
	    }
	    ...
	    $(btnsDiv).find("#desc").html((stream.owner.res && stream.owner.res.platType || "") + ":" + stream.id + ":" + (stream.vcodes || stream.optimalVideoCodecs || "-") + " < " + (stream._webrtc && stream._webrtc.getRtcId() || "-") + ":" + (stream._webrtc && stream._webrtc.optimalVideoCodecs || "-"));
	    ...
	    if (video) {
	        (function () {
	            var displayHW = function displayHW(video) {
	            ...
	            //IE video 被替换成了Object了，所以 video找不到了
	            if (stream.located()) {
	                video.muted = true;
	            }
	...
	btnEnter.onclick = function () {
	    ...
	    service.openUserMedia(pubS).then(function success(_user, stream) {
	        //成功 attachMediaStream(video, mediaStream);
